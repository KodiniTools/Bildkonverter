...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initAudioContextAndAnalyser() {
    ...
    if (!recorder) {
      recorder = new Recorder(
        ...
        { prepareRecBtn, stopRecBtn, statusBox },
        updateUIState
      );
    }
  ...
  function drawVisualizer() {
    ...
    const shouldDrawVisualizerEffect =
      isVisualizerEnabled && !audioPlayer.muted && isActuallyPlaying;
    ...
    if (recorder && (recorder.isActive || recorder.isPrepared)) {
      ...
      if (shouldDrawVisualizerEffect && Visualizers[visualizerSelect.value]) {
        ...
        Visualizers[visualizerSelect.value].draw(
          recordingCtx,
          dataArray,
          bufferLength,
  ...
  function setupEventListeners() {
    ...
    if (bgImageDropdownBtn) {
      bgImageDropdownBtn.addEventListener('click', () => {
        if (bgImageDropdown) {
          bgImageDropdown.style.display =
            bgImageDropdown.style.display === 'none' ? 'block' : 'none';
        }
      });
    }
    document.addEventListener('click', (e) => {
      if (
        bgImageDropdownBtn &&
        !bgImageDropdownBtn.contains(e.target) &&
        bgImageDropdown &&
        !bgImageDropdown.contains(e.target)
      ) {
        bgImageDropdown.style.display = 'none';
      }
    });
